...
module.exports = class Model {
      ...
      async then(resolve, reject) {
        try {
          ...
          if (results.rows.length) {
            const result = modelInstance._attachInstanceFunctions(results.rows[0]);
    ...
    return {
      ...
      async then(resolve, reject) {
        try {
          ...
          const results = await modelInstance._readonlyPool.query(query, params);
          resolve(modelInstance._attachInstanceFunctions(results.rows));
        } catch (ex) {
          ex.stack += stack;
          reject(ex);
        }
    ...
    return {
      ...
      async then(resolve, reject) {
        try {
          ...
          return resolve(Number.parseInt(result.rows[0].count));
        } catch (ex) {
          ex.stack += stack;
          reject(ex);
        }
  ...
  async create(values, {
    ...
    try {
      ...
      if (returnRecords) {
        if (_.isArray(values)) {
          return this._attachInstanceFunctions(results.rows);
        }

        if (results.rows.length) {
          return this._attachInstanceFunctions(results.rows[0]);
        }
    ...
    return {
      ...
      async then(resolve, reject) {
        try {
          ...
          if (returnRecords) {
            return resolve(modelInstance._attachInstanceFunctions(result.rows));
          }
   ...
   * @private
   */
  _attachInstanceFunctions(results) {
    if (_.isNil(results)) {
      return results;
    }

    if (!this._instanceFunctions.length) {
      return results;
    }
