...
module.exports = class Model {
  ...
  constructor({
    ...
    for (const [name, value] of Object.entries(this._schema.attributes)) {
      if (_.isFunction(value)) {
        this._instanceFunctions.push([name, value]);
      ...
      async then(resolve, reject) {
        try {
          ...
          if (results.rows.length) {
            const result = modelInstance._attachInstanceFunctionsAndShapeData(results.rows[0]);
    ...
    return {
      ...
      async then(resolve, reject) {
        try {
          ...
          const results = await modelInstance._readonlyPool.query(query, params);
          resolve(modelInstance._attachInstanceFunctionsAndShapeData(results.rows));
        } catch (ex) {
          ex.stack += stack;
          reject(ex);
        }
  ...
  async create(values, {
    ...
    try {
      ...
      if (returnRecords) {
        if (_.isArray(values)) {
          return this._attachInstanceFunctionsAndShapeData(results.rows);
        }

        if (results.rows.length) {
          return this._attachInstanceFunctionsAndShapeData(results.rows[0]);
        }
    ...
    return {
      ...
      async then(resolve, reject) {
        try {
          ...
          if (returnRecords) {
            return resolve(modelInstance._attachInstanceFunctionsAndShapeData(result.rows));
          }
   ...
   * @private
   */
  _attachInstanceFunctionsAndShapeData(results) {
    if (_.isNil(results)) {
      return results;
    }

    if (!this._instanceFunctions.length && !this._floatProperties.length && !this._intProperties.length) {
      return results;
    }

    if (_.isArray(results)) {
      for (const result of results) {
        this._attachInstanceFunctionsAndShapeDataToResult(result);
      }
    } else {
      this._attachInstanceFunctionsAndShapeDataToResult(results);
    }
   ...
   * @private
   */
  _attachInstanceFunctionsAndShapeDataToResult(result) {
    if (_.isNil(result)) {
      return;
    }

    for (const [name, fn] of this._instanceFunctions) {
